...
export default function ViewPreregistrationPage() {
    ...
    const handleEnroll = async ({ offeredCourseId, offeredCourseSectionId }: any) => {
        try {
    ...
    const handleWithdraw = async ({ offeredCourseId, offeredCourseSectionId }: any) => {
        try {
    ...
    const availableCourses: any[] = data?.map((availableCourse: any, index: number) => {
        const obj = {
            key: index,
            ...
            children: (
                <table style={{ padding: '0px 10px', borderSpacing: '10px 15px' }}>
                    {availableCourse?.offeredCourseSections?.map((section: any, index: number) => (
                        <tr key={index}>
                            <td style={{ width: '30%' }}>
                                <strong>Sec - {section?.title}</strong>
                            </td>
                            <td style={{ width: '30%' }}>
                                Enrolled - ({section?.currentEnrolledStudent}/{section?.maxCapacity}
                                )
                            </td>
                            <td style={{ width: '30%' }}>
                                <table
    ...
    const isAtLeastOneCourseTaken =
        availableCourses?.filter((el: any) => el.isTaken === true).length > 0 ? true : false;
    ...
    return (
        <>
            ...
            {availableCourses?.length > 0 ? <CustomCollapse items={availableCourses} /> : null}
