...
export default function DatePicker({
    ...
    useEffect(() => {
        ...
        if (value) {
            ...
            if (!parsedDefaultDate.isValid()) {
                ...
                !!disableBeforeDate &&
                parsedDefaultDate.isSameOrBefore(moment(disableBeforeDate, formatDate), 'day')
            ) {
                setInvalidDateError(
                    `Please enter a date after ${moment(disableBeforeDate, formatDate).format(
                        formatDate,
                    )}`,
                );
                return;
                ...
                parsedDefaultDate.isValid() &&
                !!disableAfterDate &&
                parsedDefaultDate.isSameOrAfter(moment(disableAfterDate, formatDate), 'day')
            ) {
                setInvalidDateError(
                    `Please enter a date before ${moment(disableAfterDate, formatDate).format(
                        formatDate,
                    )}`,
                );
                return;
    ...
    const renderWeekDays = () => {
        return generateWeekDays({ startOfWeekDay: startOfWeekDay }).map((weekDay, index) => (
        ...
        return (
            <>
                ...
                {daysInMonth.map((day) => {
                    ...
                    const isSelected = day.isSame(moment(selectedDate, formatDate), 'day');

                    const isToday = day.isSame(today, 'day');
                    const isDisabledBefore =
                        disableBeforeDate && day.isBefore(moment(disableBeforeDate, formatDate));
                    const isDisabledAfter =
                        disableAfterDate && day.isAfter(moment(disableAfterDate, formatDate));
    ...
    return (
        ...
        /> */}
                <input
                    ...
                    id={id}
                    name={name}
                    value={selectedDate ? moment(selectedDate, formatDate).format(formatDate) : ''}
          ...
          ${fieldClassName}
          ${disabled ? `${visibleBorder && 'disabled:border-gray-200 border-opacity-10'}` : ''}
            ${
                !!error
                    ? 'border-red-500 focus:border-red-500'
                ...
                >
                    ...
                    {renderComponent === 'month' && (
                        <section data-auto={`month_component`}>
                            ...
                            <header className="flex items-center justify-center">
                                <h3 className={`text-center mb-2 text-primary text-lg font-medium`}>
                                    Select Month
                                </h3>
                            </header>
                            {/* MONTH LIST */}
                    ...
                    {renderComponent === 'year' && (
                        <section data-auto={`year_component`}>
                            ...
                            {/* YEAR LIST */}
                            <section data-auto={`year_list`} className="grid grid-cols-3 mt-4">
